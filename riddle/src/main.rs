            ...
            State::Replying { mut plan, next, mut rx } => {
                ...
                if Instant::now() >= next {
                    ...
                    if plan.stroke_i >= plan.strokes.len() && rx.is_none() {
                        ...
                        // except mid-game turns, which would only clutter the
                        // memory with board after board.
                        if !turn_failed && !turn_reply.is_empty() && !game_on {
...
/// must never hang on the spirit's cooperation (or a slow turn).
fn wants_to_stop(transcript: &str) -> bool {
    let t = transcript.to_lowercase();
    ["stop play", "stop the game", "stop this game", "quit the game", "let's stop",
     "lets stop", "done playing", "no more game", "enough of this game", "end the game"]
        .iter()
        .any(|p| t.contains(p))
}
