    ...
    pub fn advance(&mut self, full: &str, done: bool) -> Vec<Result<Event, String>> {
        ...
        while let Some(rel) = full[self.delivered..bound].find(SHOW_OPEN) {
            ...
            if low.starts_with("ink@page:") {
                ...
                // Whole words only: ⟦game, my friend⟧ must not read as an
                // ending ("friend" contains "end"), while the model's
                // paraphrases of ⟦game over⟧ still should.
                let over = low
                    .split(|c: char| !c.is_ascii_alphanumeric())
                    .any(|w| {
                        matches!(w, "over" | "end" | "ends" | "ended" | "off" | "done" | "finished" | "stopped")
                    });
                self.emitted_any = true;
...
mod tests {
    ...
    #[test]
    fn game_directive_matches_whole_words_only() {
        // Flavor text inside the directive must not flip it to "over"
        // by substring accident…
        for body in ["game, my friend", "game of wits", "game — defending champion"] {
            let mut p = StreamParser::new(Vec::new());
            let ev = drain(p.advance(&format!("\u{27e6}{body}\u{27e7} Your move."), true));
            assert_eq!(ev[0], Event::Game(true), "{body:?} -> {ev:?}");
        }
        // …while honest paraphrases of an ending still end it.
        for body in ["game over", "game ended", "game done", "game finished", "game off"] {
            let mut p = StreamParser::new(Vec::new());
            let ev = drain(p.advance(&format!("Well played. \u{27e6}{body}\u{27e7}"), true));
            assert!(ev.contains(&Event::Game(false)), "{body:?} -> {ev:?}");
        }
    }
